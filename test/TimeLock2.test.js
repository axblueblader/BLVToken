...
const TimeLockContract = artifacts.require('TimeLock2')
...
contract('TimeLockContract', function (walletAddresses) {
    ...
    it('should add beneficiary account', async function() {
        await contract.addLockedTokens(friend,0,100,{from: me});
    ...
    it('should not let other add beneficiary account', async function() {
        let tx = contract.addLockedTokens(friend,0,100,{from: friend});
        await expectThrow(tx);
    })

    it('should not add if locktype >= 6', async function() {
        let tx = contract.addLockedTokens(friend,6,100,{from: me});
        await expectThrow(tx);
    })

    it('should not let add account after switch', async function() {
        await contract.stageSwitch();
        let tx = contract.addLockedTokens(friend,0,100,{from: me});
        await expectThrow(tx);
    })
